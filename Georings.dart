...
List<dynamic> _collectGeohashesInRings(double latitude, double longitude, double radiusMiles) {
  ...
  while (findingTargetHash) {  // Iterate throught each ring until the targhet geohash is found
    ...
    for (var gh in currentRing) {  // There are 8 sides to every ring and 8 cases to check for to no which neighbors to add to the next ring
      ...
      if (hashCounter > bottomLeftCorner && hashCounter < bottomLeftCorner + ring * 2) { // add the top left as many times as there are hashes in the side of the current ring
...
main() {
  ...
  print(_collectGeohashesInRings(45.12345, 45.12345, 1));
}
